...
void	ft_xml_xmp_to_img(t_map	*map)
{
	char	*player;
	char	*item;
	char	*exit;
	...
	map->m_player = mlx_xpm_file_to_image(map->mlx, player, \
					&map->font_w, &map->font_h);
	map->m_item = mlx_xpm_file_to_image(map->mlx, item, \
					&map->font_w, &map->font_h);
	map->m_exit = mlx_xpm_file_to_image(map->mlx, exit, \
					&map->font_w, &map->font_h);
	map->m_wall = mlx_xpm_file_to_image(map->mlx, wall, \
					&map->font_w, &map->font_h);
	map->m_empty = mlx_xpm_file_to_image(map->mlx, empty, \
					&map->font_w, &map->font_h);
}

static void	ft_put_image_window(t_map *map, void *img_ptr, int i, int j)
{
